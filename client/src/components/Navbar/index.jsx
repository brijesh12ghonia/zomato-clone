...
const SmallNav = ({ user, isDropdownOpen, setIsDropdownOpen }) => {
  ...
  return (
    <div className="flex w-full items-center justify-between lg:hidden">
      ...
      <div className="flex items-center gap-3 relative">
        ...
        { user?.fullName ? (
          <>
            <div onClick={ () => setIsDropdownOpen((prev) => !prev) } className=" border border-gray-300 text-zomato-400 w-9 h-9 rounded-full">
            ...
            { isDropdownOpen && (
              <div className="absolute shadow-lg py-3 -bottom-14 w-full z-20 flex flex-col gap-2 bg-white border border-gray-200">
    ...
    <div className="w-full items-center justify-between hidden lg:flex px-14">
      ...
      <div className="flex items-center gap-3 relative">
        { user?.fullName ? (
          ...
          <>
            <span onClick={ () => setIsDropdownOpen((prev) => !prev) } className="border p-2 border-gray-400 rounded-full">
              <FaUserAlt color="rgb(239, 79, 95)" className="w-full h-full" />
            </span>
            { isDropdownOpen && (
  ...
  return (
    <>
      <nav className="p-4 lg:py-2 flex bg-white shadow-md lg:shadow-none lg:border-b-2 border-gray-100 w-full items-center">
        <SmallNav user={ user } setIsDropdownOpen={ setIsDropdownOpen } isDropdownOpen={ isDropdownOpen } />
